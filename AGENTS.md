...
- すべて**日本時間(JST / Asia/Tokyo)**で判断する。エピソードの日付 = 実行時点の JST の日付。
  このジョブは 00:15 JST に走るので、日付は必ず「今日」であって前日ではない。
  UTCの日付を使うと1日ずれる。必ず `TZ=Asia/Tokyo date` で確認する。
- **曜日は毎回カレンダーで確認する**。冒頭の挨拶・タイトル・`description` の日付と曜日を全部そろえる。
  (2026-08-29の回で、土曜日を「金曜日」と言っていた事故を受けて追加)
